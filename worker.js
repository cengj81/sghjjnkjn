...
def get_best_candidate():
    global global_node_reservoir, dead_ips, target_country, tun_main, tun_backup, COUNTRY_POOL
    with reservoir_lock:
        ...
        # 优先用当前目标国
        candidates = [n for n in all_pool_nodes if n["country"] == target_country and n["ip"] not in dead_ips and n["ip"] not in active_ips]

        if not candidates:
            # 当前国家黑名单内还有节点，先解锁黑名单救场
        ...
        if not candidates:
            # 当前国家彻底枯竭，自动轮换到下一个国家
            other_countries = [c for c in COUNTRY_POOL if c != target_country]
            random.shuffle(other_countries)
            for alt in other_countries:
                alt_candidates = [n for n in all_pool_nodes if n["country"] == alt and n["ip"] not in dead_ips and n["ip"] not in active_ips]
                if alt_candidates:
                    print(f"[*] ⚡ [{target_country}] 完全枯竭，自动轮换目标国 -> [{alt}]", flush=True)
                    target_country = alt
